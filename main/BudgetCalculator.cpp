...
uint32_t BudgetCalculator::calculateDaysBetweenDate(date::year_month_day const &start, date::year_month_day const &end) noexcept {
  auto const dayDiff = date::sys_days(end) - date::sys_days(start);
  uint32_t const days = static_cast<uint32_t>(dayDiff.count()) + 1U;
  return days;
}

BudgetCalculator::BudgetRange BudgetCalculator::filterBudgetList(std::vector<IBudgetDB::Budget> const &allBudget, date::year_month_day const &start, date::year_month_day const &end) noexcept {
